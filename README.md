...
多人协作时偶尔会在 `git merge` 或 `git pull` 时遇到冲突，处理流程建议如下：

1. **先阅读冲突标记**：冲突文件会出现 `<<<<<<< ours` / `=======` / `>>>>>>> theirs` 标记，分别代表当前分支和待合并分支的内容；不要直接“Accept Both Changes”，而是逐段理解双方修改的意图。
2. **决定保留策略**：
   - 如果两段修改互不冲突（例如分别新增不同配置项），可以手动整合为一段，并在必要时保留双方内容；
   - 如果语义相斥（如同一函数的不同实现），需根据需求选择一方，或重写出新的逻辑。
3. **编译 / 运行自检**：解决冲突后务必重新 `cargo fmt`、`cargo check`（或前端构建）确保代码仍可编译、测试。
4. **提交说明**：`git add` 被修改的文件后，附上简短 commit message 说明冲突处理方式，方便后续排查。

通过仔细分析而非“一键接受全部”可以避免引入重复代码、语义错误或遗失他人改动，降低集成风险。
